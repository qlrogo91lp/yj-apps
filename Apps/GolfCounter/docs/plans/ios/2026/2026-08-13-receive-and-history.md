...
**참조 spec:** `docs/specs/ios/2026/2026-08-13-history-stats-design.md` (§2 화면 구조, §3 용어·집계 규칙, §4 기록 탭, §6 데이터 흐름, §7 파일 구조, §8 에러 처리, §9 테스트)

**선행 plan:** ④ `docs/plans/watch/2026/2026-08-05-round-transmission.md` — **반드시 먼저 머지되어 있어야 한다.** 이 plan은 ④가 만드는 `RoundCompletedMessage`·`RoundMetrics`·`RoundSnapshot.id/holeCount`를 그대로 쓴다. ④가 없으면 Task 2부터 컴파일되지 않는다.

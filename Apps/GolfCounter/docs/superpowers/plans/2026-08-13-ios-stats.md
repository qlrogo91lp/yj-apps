...
**선행 plan:** ⑤ `docs/superpowers/plans/2026-08-13-ios-receive-and-history.md` — **반드시 먼저 머지되어 있어야 한다.** 이 plan은 ⑤가 만든 `GolfRound.isFullRound`·`GolfRound.recordedHoleCount`·`StatCard`·`EmptyRoundsView`·`HistoryView`를 그대로 쓴다.
...
- Produces: `summary(from:)`가 `puttsPerHole`·`distribution`(항상 4개, `Bucket.allCases` 순서)·`parPerformance`(항상 3개, par 3·4·5 순서)를 채운다
...
그리고 `private func average(_:)` 위에 헬퍼 네 개를 추가한다:

```swift
    private typealias Hole = (par: Int, score: Int, putts: Int)

    /// 파가 기록되고(par > 0) 한 타 이상 친(score > 0) 홀만 모은다 (spec §3).
    /// score == 0인 홀을 넣으면 `score - par`가 -par가 되어 버디로 잘못 집계된다.
    private func countedHoles(in rounds: [GolfRound]) -> [Hole] {
        rounds.flatMap { round -> [Hole] in
            (0 ..< round.holeScores.count).compactMap { index in
                guard index < round.holePars.count else { return nil }
                let par = round.holePars[index]
                let score = round.holeScores[index]
                guard par > 0, score > 0 else { return nil }
                let putts = index < round.puttCounts.count ? round.puttCounts[index] : 0
...
```bash
make lint && make format
git add iOSApp/Features/Stats/StatsViewModel.swift iosTests/Features/Stats/StatsViewModelTests.swift
...
**Interfaces:**
- Consumes: `StatsViewModel`(Task 1·2), 차트 3종(Task 4), `StatCard`·`EmptyRoundsView`(plan ⑤), `ScoreFormat.relativeToPar(_:)`·`averageRelativeToPar(_:)`
- Produces: `StatsView()`

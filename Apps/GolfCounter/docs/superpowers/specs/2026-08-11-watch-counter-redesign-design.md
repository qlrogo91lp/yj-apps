...
> **2026-08-14 개정** — 하단 조작행을 해체했다. 모드 버튼은 헤더 가운데로, 홀 화살표는 링 좌우로
> 옮기고, 표시 전용인 홀 번호·누적 타수는 링 중앙으로 넣었다. 링 원반이 이 화면의 메인 탭
> 타깃인데 하단 행이 그 세로 예산(46mm 기준 38pt)을 잠식하고 있었기 때문이다 — 해체로
> 탭 원반이 63pt에서 90pt로 커졌다.

```
┌────────────────────────┐
│ (Par 4) [스윙 모드] (↩) │  헤더 — 버튼만 모은 행
│                         │
│      ╭───────────╮      │
│      │ 7번 홀·41타 │     │  링 — 파 칸수만큼 분할
│ ( ‹ )│     5     │( › ) │  안쪽 원반 탭 = 스트로크 +1
│      │    +3     │      │  화살표는 링 좌우 세로 중앙
│      ╰───────────╯      │
└────────────────────────┘
...
| 가운데 | `스윙 모드` / `퍼팅 모드` (알약) | **버튼** — 모드 토글 (§모드 버튼) |
| 오른쪽 | `↩` (원형) | **버튼** — 취소 (§취소 버튼) |

헤더는 **버튼만 모은 행**이다. 홀 번호·누적 타수는 표시 전용 텍스트라 버튼과 자리를 다툴
이유가 없어 링 중앙으로 내려갔고, 그 덕에 하단 조작행이 통째로 사라졌다.

Par·취소·홀 화살표 네 개는 같은 회색 원형(`circleButtonSize`, `CircleIconButton` 계열)으로
통일한다 — **회색 원형 = 보조 조작**이라는 규칙이다. 색을 가지는 것은 링과 모드 알약뿐이라,
색이 곧 "카운트에 관여한다"는 신호가 된다.

### 링 존

- 첫 줄 작은 글씨 = **홀 번호 · 라운드 누적 총 타수** (`snapshot.totalStrokes`, 현재 홀까지 포함)
- 가운데 큰 숫자 = **현재 홀 타수**
- 그 아래 작은 글씨 = **라운드 누적 파 대비** (`+3` / `E` / `-2`)
- 탭 타깃은 **링 안쪽 원반**으로 한정한다. 링 호와 그 바깥은 탭 영역이 아니다. 중앙 텍스트는
  전부 표시 전용이라 탭 타깃과 충돌하지 않는다.
- 좌우 화살표 `‹`/`›` = `goToPreviousHole()`(첫 홀에서는 비활성) / `goToNextHole()`. 링 좌우
  세로 중앙에 붙는다 — 세로 공간을 쓰지 않는 자리라 링 크기와 다투지 않지만, 대신 **가로가
  새 예산이 된다** (§10).

누적 타수에 `+`를 붙이지 않고 `41타`로 쓴다. 골프에서 `+`는 오버파를 뜻하는 기호이므로
`+41`은 "41 오버"로 읽힌다. 바로 아래 `+3`(파 대비)과 부호가 겹쳐 서로 다른 의미가 되는 것을
피한다. 큰 숫자와 누적 타수는 같은 단위지만, 누적 쪽은 홀 라벨과 `7번 홀 · 41타`로 한 줄에
묶여 있어 범위 혼동이 없다.

**폭 제약**: 중앙 텍스트의 가용 폭은 안쪽 원반에서 첫 줄 높이가 지나는 원의 현이다. 40mm(원반
70pt)에서는 전체 표기 `7번 홀 · 41타`가 이 현보다 넓으므로 `tight` 세트에서 `H7 · 41타`로
축약한다. `CountingSizing`의 `usesShortHoleLabel` 플래그가 이를 결정한다.
...
| 변형 | 모양 | 라벨 | 조건 |
|---|---|---|---|
| wide | 알약 | `스윙 모드` / `퍼팅 모드` | 헤더 양끝 원형 버튼을 뺀 남는 폭이 `modeWideWidth` 이상 |
...
하단 중앙(엄지 최적 자리)을 포기하고 헤더로 올린 트레이드오프: 모드 전환은 홀당 한두 번이고
— 홀 이동 시 스윙으로 자동 복귀(§3)하므로 실질적으로는 그린 진입 시 1회 — 스트로크 탭은
홀당 네댓 번 이상이다. 저빈도 조작의 접근성보다 최고빈도 탭 타깃의 크기를 우선한다.
...
> **2026-08-14 개정** — 아래 표는 최초 구현 당시 기록이다. 이후 §5 개정으로
> `HoleNavigation.swift` · `UndoButton.swift`는 삭제되고, 네 원형 버튼이 공유하는
> `CircleIconButton.swift`가 신설됐다. 화살표는 `CountingView`의 링 존이 직접 배치한다.
...
> **2026-08-14 개정** — 하단 조작행 해체(§5)에 맞춰 필드를 다시 짰다. `parButtonSize` ·
> `undoSize` · `arrowSize`는 하나의 `circleButtonSize`로 통합되고(네 원형 버튼이 같은 크기),
> `headerHeight`는 별도 필드 없이 `circleButtonSize`가 겸한다. 홀 라벨이 링 중앙으로
> 들어가면서 `holeInfoFont`가 새로 생겼다.

`ViewThatFits`가 regular → compact → tight 순으로 시도하는 구조는 유지하며, **기기 모델을 분기하지 않는다.** 다만 화살표가 링 좌우로 오면서 가로도 예산이 됐으므로, `ScoringView`의 판정 축은 `[.horizontal, .vertical]` 두 축이다.

| 필드 | regular | compact | tight |
|---|---|---|---|
| `circleButtonSize` | 30 | 26 | 25 |
| `headerFont` | 13 | 12 | 11 |
| `modeFont` | 12 | 11 | 10 |
| `modeHeight` | 26 | 24 | 22 |
| `modeWideWidth` | 66 | 62 | 58 |
| `ringDiameter` | 100 | 85 | 78 |
| `ringStroke` | 10 | 9 | 8 |
| `overflowStroke` | 4 | 3.5 | 3 |
| `overflowGap` | 3 | 2.5 | 2 |
| `holeInfoFont` | 13 | 12 | 11 |
| `scoreFont` | 36 | 32 | 28 |
| `relativeFont` | 12 | 11 | 10 |
| `usesShortHoleLabel` | false | false | true |
| `spacing` | 5 | 4 | 3 |

**`ringDiameter`가 아니라 `outerRadius*2`가 실제 렌더 값이다** — `StrokeRing`은 초과 링이 비어 있어도 그 공간을 항상 프레임에 예약하기 때문이다. `ringDiameter`는 `outerRadius*2`가 예산 안에 들어오도록 역산한 값이다.

### 링 크기를 정하는 제약

바깥 초과 링까지 포함한 실제 반지름은 다음과 같다:

```
outerRadius = ringDiameter/2 + ringStroke/2 + overflowGap + overflowStroke
```

- **세로**: `circleButtonSize + spacing + outerRadius*2`가 실측 가용 높이 이내 —
  regular 159 ≤ 167.5 / compact 136 ≤ 152 / tight 124 ≤ 138.5.
- **가로**: `circleButtonSize*2 + spacing*2 + outerRadius*2`가 각 세트의 가장 좁은 대상
  기기 폭에서 좌우 패딩 8pt를 뺀 값 이내 — regular 194 ≤ 197(Ultra 49mm) /
  compact 166 ≤ 168(41mm) / tight 152 ≤ 154(40mm).

가용 세로가 커진 대신(하단 행 소멸) 화살표가 옆으로 와서, **46mm 이상에서는 세로가 아니라
가로가 링 크기를 묶는다.** 세로 예산이 화면 높이가 아니라 중첩 TabView 크롬을 뺀 실측치인
이유는 코드 주석과 `CountingSizingTests` 참조.

세 세트 모두 46mm·44mm·40mm 시뮬레이터에서 실측 확인했다 (2026-08-14). 탭 원반
(`innerDiameter`)은 90 / 76 / 70pt — 개정 전 63 / 54 / 49pt에서 면적 기준 약 2배가 됐다.

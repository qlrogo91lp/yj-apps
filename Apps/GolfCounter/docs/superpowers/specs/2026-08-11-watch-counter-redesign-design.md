...
8. **취소 버튼은 링을 절대 가리지 않는다.** 링이 이 화면의 시각적 핵심이므로, 취소는 링 바깥 빈 코너에 둔다.
...
**링을 가리지 않는다.** 이전 홀 화살표(`‹`) 바로 위, 링 바깥의 빈 코너에 뜬다. 아이콘만 있는 원형이며 "취소" 텍스트는 넣지 않는다.
...
링이 원이라 그 바운딩 박스의 네 모서리는 원래 비어 있다. 취소는 그중 왼쪽 아래를 쓰므로 **세로 공간을 추가로 소모하지 않는다.** 부수 효과로 링 아래에 취소용 여유를 남길 필요가 없어져 링 지름을 regular 기준 약 13pt 키울 수 있다.

되돌릴 게 없을 때는 자리를 예약하지 않고 그냥 사라진다 — 주변이 빈 공간이라 레이아웃이 밀리지 않는다.

**왼쪽을 고른 이유**: 되돌리기가 "뒤로" 계열이라 `‹`(이전 홀)와 같은 쪽에 묶이는 편이 읽기 자연스럽다. 대신 두 버튼이 세로로 인접하므로 최소 간격(`undoGap`, §10)을 확보한다 — `‹`를 잘못 누르면 홀이 바뀌면서 `strokeHistory`가 비워져 **되돌릴 기회 자체가 사라지기** 때문이다. 반대 방향 오터치(취소를 잘못 누름)는 한 타 다시 넣으면 복구되므로 대가가 훨씬 작다.

탭 타깃이 링 안쪽 원반으로 한정되어 있고 취소는 링 바깥에 있으므로, 둘 사이에는 아무것도 아닌 빈 영역이 있다. 어느 쪽을 빗맞혀도 +1이 잘못 들어가지 않는다.
...
세로 합계: regular 224pt / compact 192pt / tight 160pt. 취소가 링 바깥 코너에 들어가므로 세로 합계에 취소 몫이 없다.

### 취소 코너 배치 제약

취소는 `‹` 바로 위에 놓이되, 링과 겹치지 않아야 한다. 링 중심에서 취소 중심까지의 거리가 다음을 만족해야 한다:

```
distance > ringDiameter/2 + ringStroke/2 + undoSize/2 + 4
```

만족하지 않으면 `ringDiameter`를 줄인다. 링을 키우는 것보다 링을 안 가리는 것이 우선이다(§3-8).

`undoGap`은 취소와 `‹` 사이 세로 최소 간격이다 — `‹` 오터치가 `strokeHistory`를 날려버리므로 이 간격은 줄이지 않는다.

**이 값들은 출발점이며 구현 중 실측으로 조정한다.** 특히 40mm(162×197pt)에서 위 제약이 성립하는지, 그리고 `modeWideWidth`가 안 들어가 compact 변형으로 떨어지는지를 확인해야 한다.

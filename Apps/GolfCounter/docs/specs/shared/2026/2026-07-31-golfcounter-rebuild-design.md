...
**공통 기반 작업을 먼저 묶어 진행**하고, 그 위에서 나머지를 플랫폼별로 나눈다. plan 문서는 `docs/plans/`에 flat하게 두고 **파일명에 `common-`/`watch-`/`ios-` prefix**로 구분한다 (서브폴더 분리는 하지 않음). 전송처럼 양쪽에 걸치는 기능 작업은 발신(워치)/수신(iOS)으로 나눠 각 플랫폼 plan에 담는다.

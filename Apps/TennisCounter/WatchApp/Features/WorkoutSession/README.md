...
### WatchConnectivityService (Shared/Services/WatchConnectivityService.swift)

iOS·Watch 공유 싱글턴. `WCSession.default`를 래핑해 메시지를 타입별로 `@Published` 프로퍼티에 파싱·발행한다. ViewModel은 Combine으로 이 프로퍼티를 구독한다.

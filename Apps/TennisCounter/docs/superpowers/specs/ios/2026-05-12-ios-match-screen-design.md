...
- 운동 시간 — `.yellow`
- kcal — `.white` (기본색)
- bpm — `.white` (기본색) + `heart.fill` 아이콘 `.red`
...
- 좌: 나 — `.green.opacity(0.15)` 배경, 탭 시 포인트 추가
- 우: 상대 — `.orange.opacity(0.15)` 배경, 탭 시 포인트 추가
- 포인트 점수 (40 / 15): 각 반쪽 중앙, 나 `.green` / 상대 `.orange`, `.system(size: ~70, weight: .heavy)`

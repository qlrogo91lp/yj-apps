...
| `iosTests/iosTests.swift` | ScoreViewModel 버그 재현 + 신규 기능 테스트 |
| `watchosTests/watchosTests.swift` | 동일 |
...
`iosTests/iosTests.swift`의 `// MARK: - ScoreViewModel addPoint` 섹션 아래에 추가:
...
```bash
git add iosTests/iosTests.swift iOSApp/Features/Match/Score/ScoreViewModel.swift
...
`watchosTests/watchosTests.swift`에 추가:
...
xcodebuild test -project TennisCounter.xcodeproj -scheme "TennisCounter Watch App" \
  ...
  -only-testing:watchosTests/watchosTests/watchDrawAt_T6_noTie 2>&1 | grep -E "FAILED|PASSED|error:"
```
...
```bash
git add watchosTests/watchosTests.swift WatchApp/Features/Match/Score/ScoreViewModel.swift

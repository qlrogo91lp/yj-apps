...
최초 구현(회전 누적 → `for` 루프로 여러 점 적립)이 실기기에서 한 번 돌리면 세트 스코어가 4까지
올라가는 문제로 드러났다. 원인은 회전량 단위가 감도·범위에 따라 달라 예측할 수 없다는 것 —
기준값을 30 → 10으로 낮춰도 여전히 몇 바퀴를 돌려야 겨우 넘었다.

**최종 결정: `digitalCrownRotation(detent:)` 방식으로 교체.** 시스템이 "한 칸"을 정해 주므로
회전량을 직접 재지 않는다. `CrownPointGate`가 디텐트 값의 변화(증가/감소)만 보고 한 번의
제스처(크라운이 멈출 때까지)에 최대 1점만 통과시킨다. 감도는 `.low`(가장 둔함)로 두어 손목이
스쳐도 안 걸린다. 스크롤 바가 뜨는 부작용은 `.digitalCrownAccessory(.hidden)`으로 막았다.
실기기에서 정상 동작 확인 완료.

포인트 햅틱을 `.directionUp`으로 바꿔 봤으나 느낌이 튀어 `.click`으로 되돌렸다 — 약하게
느껴지지만(체감 이슈, [햅틱 플랜](2026-09-07-match-haptics.md) 참고) 유지하기로 결정.

...
struct Provider: TimelineProvider {
    func placeholder(in _: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), isWorkoutActive: false, rotationDegrees: 0)
    }

    func getSnapshot(in _: Context, completion: @escaping (SimpleEntry) -> Void) {
        let isActive = UserDefaults(suiteName: appGroupID)?.bool(forKey: workoutActiveKey) ?? false
        completion(SimpleEntry(date: Date(), isWorkoutActive: isActive, rotationDegrees: 0))
    }
    ...
    func getTimeline(in _: Context, completion: @escaping (Timeline<Entry>) -> Void) {
        ...
        if isActive {
            ...
            let entry = SimpleEntry(date: Date(), isWorkoutActive: false, rotationDegrees: 0)
...
#Preview(as: .accessoryCircular) {
    ...
    SimpleEntry(date: .now, isWorkoutActive: false, rotationDegrees: 0)
    SimpleEntry(date: .now, isWorkoutActive: true, rotationDegrees: 0)
    SimpleEntry(date: .now, isWorkoutActive: true, rotationDegrees: 45)
    SimpleEntry(date: .now, isWorkoutActive: true, rotationDegrees: 90)
}

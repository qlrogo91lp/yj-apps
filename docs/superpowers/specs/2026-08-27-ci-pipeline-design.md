...
**실증 (문서 제외 도입 후, 2026-09-01).** PR #5 머지 커밋 `7a02427` — `ci.yml` 과 이 스펙 문서가
함께 바뀐 경우다. 문서는 걸러졌지만 워크플로 파일이 남아 "루트 설정 → 전부" 규칙에 걸렸다.

```
변경: .github/workflows/ci.yml
      docs/superpowers/specs/2026-08-27-ci-pipeline-design.md
판별: kit=true golf=true tennis=true
결과: 7 job 전부 실행 (전부 성공)
```

**필터가 동작했는데도 전 job 이 도는 것이 맞는 동작이다.** 워크플로 자체를 바꾼 커밋이므로
전 대상을 검증해야 한다. 문서 제외의 효과는 여기서 관찰되지 않는다 — `.github/` 를 건드리는
변경은 언제나 전부를 트리거하기 때문이다.

효과는 **문서만 바뀐 커밋**에서 나타난다. 바로 위 문단을 추가한 커밋 `e1ad2f3` 이 그 경우였고,
의도대로 동작했다.

```
--- 변경된 파일 ---
docs/superpowers/specs/2026-08-27-ci-pipeline-design.md
문서만 바뀌었다 — 빌드·테스트를 건너뛴다
판별 결과 — kit=false golf=false tennis=false
```

`변경 경로 판별` 만 **6초** 만에 끝나고 나머지 6 job 이 전부 `skipped` 로 처리됐다.
같은 커밋이 도입 전이었다면 — 루트 `docs/` 라 원래도 스킵됐겠지만 — `Packages/YJKit/docs/` 나
`Apps/*/CLAUDE.md` 였다면 6분에 걸쳐 전 앱을 빌드했을 것이다.

| | 도입 전 | 도입 후 |
|---|---|---|
| 코드 디렉터리 안 문서만 바뀐 커밋 | 7 job, 약 6분 (22 러너-분) | 1 job, 6초 |

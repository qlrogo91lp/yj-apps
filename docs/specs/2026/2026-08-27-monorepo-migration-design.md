...
- [ ] 기존 3개 레포 archive
- [x] 후속 문서 [2] CI 진행 여부 판단 — **착수 결정 (2026-08-28)**
- [ ] 후속 문서 [3] 코드 스타일 진행 여부 판단

#### CI 착수 결정 (2026-08-28)

`2026-08-27-ci-pipeline-design.md`가 요구하는 선행 조건(3·4·5단계)이 모두 충족되어 CI 구축에
착수한다. **6단계의 실기기 연동 확인이 보류 상태인 것은 블로커가 아니다** — CI는 서명 없이
시뮬레이터에서 빌드·테스트만 하므로 실기기 연동을 검증할 수 있는 수단이 애초에 없다. 두 작업의
스코프가 겹치지 않는다.

착수에 앞서 lint 기준선을 초록으로 만들었다. 5단계에서 "기준선과 동일"을 지키려고 남겨둔
tennis의 swiftformat 위반 1건(`MatchDetailSheet.swift:16 trailingCommas`)이 CI의 lint job을
첫 PR부터 실패시키기 때문이다. 전환 검증이 끝난 뒤이므로 "코드 diff 0" 원칙을 더 지킬 이유가 없다.

```
PR #1  🎨 swiftformat trailingCommas 위반 1건 수정
       make lint    golf 0 / tennis 0
       make format  golf 0/96 / tennis 0/93   (전환 시점 tennis 1/93 → 0/93)
```

CI 문서는 전환 실행 중 확인된 사실에 맞춰 함께 갱신했다 — `swift test` 불가(macOS 플랫폼 미선언),
시뮬레이터 이름 중복이 우려가 아니라 실제 재현된 문제라는 점, 확정된 공유 스킴 7개.
...
`Apps/TennisCounter/docs/ideas/ralli-update-roadmap.md`에 **"Phase 1-B 출시 시점, 앱 아이콘 리뉴얼과 함께"**
